...
var remoteApi       = require("./remote")

exports.register = function(req, res) {
	Promise.coroutine(function *() {
		var request = req.body;
		console.log("request is in register - ",JSON.stringify(request));

		var username = request.username;
		var password = request.password;

		if(!username || !password) {
			return responses.sendError(res,"username and password is required",{});
		}

		yield remoteApi.remoteSignup({username: username, password: password});


		return responses.actionCompleteResponse(res,"Registration Success",{});
	})().catch(function(error) {
		console.log("error is in register - ",error);
